...
const renderPosition = ({ onChange, value = 0 }) => {
	...
	return <RangeField detail="px" max={1000} min={-1000} step={1} onChange={onChange} value={value} />;
};
...
export const definitionMap = {
	...
	'background-image': {
		...
		preview: (v) => 'custom',
		parse: (v) => v,
		render: ({ onChange, value }) => <UrlParseField onChange={(v) => onChange(v.url)} value={value} />
	},
	'background-repeat': {
		format: (v) => v,
		preview: (v) => v,
	...
	transform: {
		...
		format: (v) =>
			`translate(${v.translate.x}px, ${v.translate.y}px) rotateZ(${v.rotate.z}deg) scale(${v.scale.x}, ${v.scale.y})`,
		preview: (v) =>
			`(${Math.round(v.translate.x)}, ${Math.round(v.translate.y)}) (${Math.round(
				v.rotate.z
			)}) (${Number(v.scale.x).toFixed(2)}, ${Number(v.scale.y).toFixed(2)})`,
		lerp: (from, to, t) => {
			return {
				translate: {
