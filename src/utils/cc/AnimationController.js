...
const AnimationController = ({ children, delay, duration, easing, format = true, keyframes = {}, time }) => {
	...
	return children(
		Object.entries(keyframes).reduce((map, [propName, keyframesArray]) => {
			...
			if (styleProp && canInterpolate(propName)) {
				const value = interpolateKeyframes(keyframesArray, ratio, styleProp.lerp, easing);
