...
const uid = (length = 6) =>
	`${Math.floor(Math.random() * Number.MAX_SAFE_INTEGER).toString(16)}`.slice(0, length);
...
export default class AnimationStore extends React.Component {
	static propTypes = {
		...
		animations: (props, propName, componentName) => {
			const err = (message) => {
				return new Error(`Props ${propName} in ${componentName}: ${message}`);
			};
			...
			for (let a = 0; a < animEntries.length; ++a) {
				...
				for (let p = 0; p < percentEntries.length; ++p) {
					...
					if (isNaN(parsePercent(percentName))) {
						return err(`${animName} has an invalid key: ${percentName}`);
					}

					// CHECK: percent styles is object
					if (!isObject(percentValue)) {
						return err(`${animName} at "${percentName}" must be an object`);
					}
	...
	constructor(props) {
		...
		Object.entries(props.animations).forEach(([name, keyframes]) => {
			...
			css += `@keyframes ${keyframesId} { ${Object.entries(keyframes).map(writeKeyframe).join(' ')} } `;
		});
